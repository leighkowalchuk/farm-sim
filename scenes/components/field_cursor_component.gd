...
func add_crop_field_cell() -> void:
	if distance > 20.0 && cell_source_id != -1:
		crop_field_tilemap_layer.set_cells_terrain_connect([cell_position], terrain_set, terrain, true)
		
func remove_crop_field_cell() -> void:
	if distance < 20.0:
		crop_field_tilemap_layer.set_cells_terrain_connect([cell_position], 0, -1, true)
